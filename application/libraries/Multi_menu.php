...
class Multi_menu {
	 ...
	 * @var string
	 */
	private $parent_anchor            = '<a href="%s" class="expand">%s<span class="pull-right-container"><i class="fa fa-angle-left pull-right"></i></span></a>';
	...
	{	    
	    ...
	    foreach ($items as $item)
	    {
	        ...
	        if ( isset($item[$this->menu_label], $item[$this->menu_key]) ) 
	        {
		        ...
		        if ($icon) 
		        {
		        	$icon = "<i class='{$icon}'></i><span>";
		        	$label = trim( $this->icon_position == 'right' ? ($label . " " . $icon ) : ($icon . " " . $label.'</span>') );
		        }
